...
//#######################################
#include "mozilla/dom/testapi/Testapi.h"
//#######################################
...
//#####################################################
  NS_IMPL_CYCLE_COLLECTION_TRAVERSE(mTestapi)
//#####################################################
#ifdef MOZ_B2G_RIL
...
// 属性的实现函数
testapi::Testapi*
Navigator::GetMozTestapi(ErrorResult& aRv)
{
  printf("aiyan GetMozTestapi###############################################\n");
  if(!CheckPermission("testapi")){
  printf("aiyan GetMozAaaTest CheckPermission return false###################################################\n");
  aRv.Throw(NS_ERROR_UNEXPECTED);
  ...
  if(!mTestapi){
    mTestapi =new testapi::Testapi(mWindow);
  }
  else{
    printf("mAaaTest exist just return#########################################################################\n");
    NS_ERROR("mAaaTest exist just return#########################################################################");
  }
  return mTestapi;
}
